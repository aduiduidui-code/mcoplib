...
def _torch_neox_rope(q, k, positions, cos_sin_cache, head_size):
    # cos_sin_cache: [max_position, head_size] where first half ([:head_size//2])
    # is cos, second half ([head_size//2:]) is sin. NeoX style: pair
    # (i, i+head_size//2). In-place on q and k.
...
class Sgl_rotary_embedding_runner(OpBenchmarkBase):
    ...
    def prepare_and_get_launcher(self, dev_id, tc_s):
        ...
        return self.make_launcher(
            dev_id, torch.ops.sgl_kernel.rotary_embedding,
            positions, q, k, self.head_size, cos_sin_cache, self.is_neox
        )
    ...
    def run_verification(self, dev_id):
        ...
        q_ref, k_ref = _torch_neox_rope(q_in, k_in, positions, cos_sin_cache, self.head_size)
